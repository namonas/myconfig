...
def CallAddr2Line(lib, addr):
  uname = "linux-x86"
  if lib != "":
    cmd = "/data/omapts/linux/arm-tc/cs-arm-2010q1/bin/arm-none-linux-gnueabi-addr2line" \
        + " -f -e " + SYMBOLS_DIR + lib \
        + " 0x" + addr
    stream = os.popen(cmd)
  ...
  if list != []:
    ...
    #cmd = "./prebuilt/" + uname + "/toolchain/arm-eabi-4.4.0/bin/arm-eabi-c++filt "\
    cmd = "/data/omapts/linux/arm-tc/cs-arm-2010q1/bin/arm-none-linux-gnueabi-c++filt "\
          + mangled_name
    stream = os.popen(cmd)
